...
import {
  CheckSquare, CheckCircle2, AlertCircle,
  RotateCcw, Eye, Edit3, Award, MinusCircle,
  Save, X, Check, HelpCircle, ShieldAlert,
  Plus, Trash2, Sliders, Square, CheckSquare2,
  ChevronLeft, ChevronRight, Layers, LayoutList, FileText, ClipboardList, Sparkles,
  Undo2, Redo2
} from 'lucide-react';
import { scoreService } from '../../services/scoreService';
import { ChoiceOption, MultipleResponseQuestionItem, MultipleResponseActivityData, QuizActivity } from '../../types/quiz';
...
const defaultQuestions: MultipleResponseQuestionItem[] = [
  {
    id: 'mr_q_1',
    prompt: '1. Enter multiple response question prompt here',
    options: [
      { id: 'mr_1_1', text: 'Option A', isCorrect: true },
      { id: 'mr_1_2', text: 'Option B', isCorrect: true },
      { id: 'mr_1_3', text: 'Option C', isCorrect: false },
      { id: 'mr_1_4', text: 'Option D', isCorrect: false },
    ],
    explanation: '',
  },
];
...
export default function MultipleResponse({ initialData, onBack, onSaveToCourse }: MultipleResponseProps) {
  const [viewMode, setViewMode] = useState<'author' | 'preview'>('author');
  const [activityTitle, setActivityTitle] = useState(
    initialData?.prompt || 'Multiple Response Activity'
  );
  const [instructions, setInstructions] = useState(
    initialData?.instructions || 'Choose all applicable options for each question.'
  );

  // 1 to 10 Questions
  const [questions, setQuestions] = useState<MultipleResponseQuestionItem[]>(() => {
    if (initialData?.data?.multipleResponse?.questions && initialData.data.multipleResponse.questions.length > 0) {
      return initialData.data.multipleResponse.questions;
    }
    if (initialData?.data?.multipleResponse?.options) {
      return [
        {
          id: 'mr_q_1',
          prompt: initialData.prompt || 'Select all resources and technologies classified as Renewable Energy Sources:',
          options: initialData.data.multipleResponse.options,
          explanation: initialData.data.multipleResponse.explanation || '',
        },
      ];
    }
  ...
  const [displayMode, setDisplayMode] = useState<'paginated' | 'single_page'>(
    initialData?.data?.multipleResponse?.displayMode || 'paginated'
  );
  const [allowPartialCredit, setAllowPartialCredit] = useState<boolean>(
    initialData?.data?.multipleResponse?.allowPartialCredit ?? true
  );

  // Scoring
  const totalCorrectCount = questions.reduce((acc, q) => acc + q.options.filter(o => o.isCorrect).length, 0);
  ...
  const totalMaxScore = totalCorrectCount * pointsPerCorrect;
  ...
  const [selectedAnswers, setSelectedAnswers] = useState<Record<string, string[]>>({}); // { [qId]: [optId1, optId2] }
  ...
  const [validationResult, setValidationResult] = useState<{
    ...
    isPassed: boolean;
    isFailed: boolean;
    questionEvaluations: Record<string, { correctPicks: number; incorrectPicks: number; isFullMatch: boolean }>;
  } | null>(null);

  useEffect(() => {
    if (initialData) {
      setActivityTitle(initialData.prompt || 'Renewable Technologies Multiple Response Checkpoint');
      setInstructions(initialData.instructions || 'Choose all applicable options. More than one choice is correct for each question.');
      if (initialData.data?.multipleResponse?.questions && initialData.data.multipleResponse.questions.length > 0) {
        setQuestions(initialData.data.multipleResponse.questions);
      }
      if (initialData.data?.multipleResponse?.displayMode) {
        setDisplayMode(initialData.data.multipleResponse.displayMode);
      }
      setAllowPartialCredit(initialData.data?.multipleResponse?.allowPartialCredit ?? true);
      ...
      const calculatedMax = totalCorrectCount * (initialData.pointsPerCorrect ?? 2);
  ...
  const applySnapshot = useCallback((state: typeof currentSnapshot) => {
    ...
    if (state.displayMode !== undefined) setDisplayMode(state.displayMode);
    if (state.allowPartialCredit !== undefined) setAllowPartialCredit(state.allowPartialCredit);
  ...
  // Authoring: Add / Remove / Modify Questions (Unlimited, min 1)
  const handleAddQuestion = () => {
    const newQ: MultipleResponseQuestionItem = {
      id: `mr_q_${Date.now()}_${Math.random().toString(36).substring(2, 5)}`,
      prompt: `${questions.length + 1}. Select all applicable options that apply:`,
      options: [
        { id: `opt_${Date.now()}_1`, text: 'Valid Option A', isCorrect: true },
        { id: `opt_${Date.now()}_2`, text: 'Valid Option B', isCorrect: true },
  ...
  const handleAddOptionToQuestion = (qId: string) => {
    setQuestions(prev => prev.map(q => {
      ...
      const newOpt: ChoiceOption = {
        ...
        text: `New Choice Option ${q.options.length + 1}`,
        isCorrect: false,
      };
  ...
  // Paste Choices Bulk Modal State
        ...
        if (destIndex < 8) {
          if (destIndex < newOptions.length) {
            newOptions[destIndex] = {
              ...newOptions[destIndex],
              text: item.text,
              ...(item.isCorrect !== undefined ? { isCorrect: item.isCorrect } : {}),
            };
          } else {
            while (newOptions.length <= destIndex && newOptions.length < 8) {
              const isTarget = newOptions.length === destIndex;
              const newId = `mr_${Date.now()}_${Math.random().toString(36).substring(2, 5)}_${newOptions.length}`;
              newOptions.push({
                id: newId,
                text: isTarget ? item.text : `Option ${String.fromCharCode(65 + newOptions.length)}`,
                isCorrect: isTarget && item.isCorrect !== undefined ? item.isCorrect : false,
              });
            }
          }
        }
      ...
      if (parsedList.length === 1) {
        ...
        setQuestions(prev => prev.map(q => {
          ...
          const updatedOptions = single.options.map((opt, oIdx) => ({
            id: q.options[oIdx]?.id || `mr_opt_${Date.now()}_${oIdx}_${Math.random().toString(36).substring(2, 5)}`,
            text: opt.text,
        ...
        const createdItems: MultipleResponseQuestionItem[] = parsedList.map((pq, idx) => ({
          id: `mr_q_${Date.now()}_${idx}_${Math.random().toString(36).substring(2, 5)}`,
          prompt: pq.prompt,
          options: pq.options.map((opt, oIdx) => ({
            id: `mr_opt_${Date.now()}_${idx}_${oIdx}`,
            text: opt.text,
            isCorrect: opt.isCorrect,
          })),
        ...
        setQuestions(prev => {
          if (prev.length === 1 && (prev[0].prompt.includes('Enter') || !prev[0].prompt.trim())) {
            return createdItems;
          }
          const next = [...prev];
  ...
  // Learner: toggle selection
  const handleToggleSelectLearner = (qId: string, optId: string) => {
  ...
  const handleCheckAllAnswers = () => {
    ...
    const questionEvaluations: Record<string, { correctPicks: number; incorrectPicks: number; isFullMatch: boolean }> = {};

    questions.forEach(q => {
      const selected = selectedAnswers[q.id] || [];
      let qCorrect = 0;
      let qIncorrect = 0;
      const qTotalCorrect = q.options.filter(o => o.isCorrect).length;

      q.options.forEach(opt => {
        const isSel = selected.includes(opt.id);
        if (isSel) {
          if (opt.isCorrect) qCorrect += 1;
          else qIncorrect += 1;
        }
      });

      totalCorrectPicks += qCorrect;
      totalIncorrectPicks += qIncorrect;

      const isFullMatch = qCorrect === qTotalCorrect && qIncorrect === 0;
      questionEvaluations[q.id] = { correctPicks: qCorrect, incorrectPicks: qIncorrect, isFullMatch };

      if (allowPartialCredit) {
        const earned = qCorrect * pointsPerCorrect;
        const ded = pointsPerCorrect > 0 ? qIncorrect * deductionPerMistake : 0;
        totalEarned += Math.max(0, earned - ded);
      } else {
        if (isFullMatch) {
          totalEarned += qTotalCorrect * pointsPerCorrect;
        }
      }
    });

    const isGradedTest = pointsPerCorrect > 0;
    const finalScore = isGradedTest ? Math.max(0, totalEarned) : 0;
    ...
    scoreService.recordQuizScore({
      ...
      quizId: initialData?.id || 'quiz_mr_01',
      quizTitle: activityTitle || 'Multiple Response Assessment',
      quizType: 'Multiple Response',
      score: finalScore,
      maxScore: totalMaxScore,
      correctCount: totalCorrectPicks,
      mistakeCount: totalIncorrectPicks,
      attempts: 1,
    });
  };
  ...
  const handleSaveToCourse = () => {
    const activity: QuizActivity = {
      id: initialData?.id || `quiz_mr_${Date.now()}`,
      name: activityTitle || 'Multiple Response Activity',
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-slate-500 dark:text-slate-400">
                Multiple Response ({questions.length} Questions • {displayMode === 'paginated' ? 'Separate Pages' : 'Single Page'})
              </span>
            </div>
            <h2 className="text-xl font-bold text-slate-900 dark:text-white">
              {activityTitle || 'Untitled Multiple Response Assessment'}
            </h2>
          </div>
        </div>
        ...
        <div className="space-y-6">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pt-3 border-t border-slate-100 dark:border-slate-800">
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  className="flex items-center gap-1.5 px-3 py-1.5 bg-slate-100 dark:bg-slate-800 hover:bg-blue-50 dark:hover:bg-blue-950/50 text-slate-700 dark:text-slate-200 hover:text-blue-600 dark:hover:text-blue-400 border border-slate-200 dark:border-slate-700 rounded-lg text-xs font-bold transition-all cursor-pointer shadow-2xs"
                  title="Paste multiple response questions with choices"
                >
                ...
                <button
                  ...
                  className="flex items-center gap-1.5 px-3 py-1.5 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-xs font-bold shadow-2xs transition-all cursor-pointer"
                >
                  <Plus size={14} />
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 pt-3 border-t border-slate-100 dark:border-slate-800">
              <div className="space-y-1.5 p-3.5 bg-slate-50 dark:bg-slate-950/60 rounded-xl border border-slate-200 dark:border-slate-800">
                ...
                <p className="text-[11px] text-slate-400 dark:text-slate-500">
                  {pointsPerCorrect > 0 ? `Total max: ${totalMaxScore} pts` : '0 pts = Non-graded practice'}
                </p>
              </div>
              ...
              <div className={`space-y-1.5 p-3.5 rounded-xl border transition-all ${pointsPerCorrect > 0
                ...
                <label className="flex items-center justify-between text-xs font-semibold text-slate-700 dark:text-slate-300">
                  <span className="flex items-center gap-1.5">
                    ...
                    <span>Deduction per wrong pick</span>
                  </span>
                  {pointsPerCorrect === 0 && <span className="text-[10px] font-medium text-slate-400">Grayed out</span>}
                </label>
                <div className="flex items-center gap-2">
                ...
                <p className="text-[11px] text-slate-400 dark:text-slate-500">
                  {pointsPerCorrect > 0 ? 'Points subtracted per false pick' : 'No deductions on non-graded'}
                </p>
              </div>
          ...
          <div className="space-y-4">
            <h4 className="text-sm font-bold text-slate-900 dark:text-white">
              Multiple Response Questions ({questions.length} total)
            </h4>
            ...
            {questions.map((q, qIndex) => (
              ...
              >
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300">
                      Options (Check all correct valid choices):
                    </label>
                    <span className="text-[11px] text-blue-600 dark:text-blue-400 font-medium">
                  ...
                  <div className="space-y-2">
                    {q.options.map((opt, optIdx) => (
                      <div
                        key={opt.id}
                        className={`p-2.5 rounded-xl border flex items-center gap-3 transition-all ${opt.isCorrect
                      ...
                      >
                        <input
                          ...
                          onChange={() => handleToggleOptionCorrect(q.id, opt.id)}
                        ...
                        <input
                          ...
                          className="flex-1 px-3 py-1 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg text-xs font-medium text-slate-900 dark:text-white outline-hidden focus:ring-2 focus:ring-blue-500"
                        />
                        {q.options.length > 2 && (
                          <button
                            type="button"
                ...
                <div className="space-y-1.5 pt-1">
                  <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300">
                    Explanation (Shown during review, optional)
                  </label>
                  <input
                    type="text"
                    value={q.explanation || ''}
                    onChange={(e) => handleUpdateQuestionExplanation(q.id, e.target.value)}
                    placeholder="e.g. Explaining the correct options... (Optional)"
          ...
          <div className="p-4 bg-gradient-to-r from-blue-50/90 to-indigo-50/90 dark:from-blue-950/40 dark:to-indigo-950/40 border border-blue-200 dark:border-blue-800 rounded-2xl flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 shadow-xs">
            <div className="space-y-0.5">
              ...
              <p className="text-xs text-slate-600 dark:text-slate-400">
                {displayMode === 'paginated' ? 'Navigate questions one-by-one or click any question tab.' : 'Answer all questions listed below.'}
              </p>
            </div>
            ...
            <div className="space-y-6 max-w-2xl mx-auto">
              <div className="flex items-center justify-between gap-2 p-2 bg-slate-100 dark:bg-slate-800/80 rounded-2xl border border-slate-200 dark:border-slate-700 overflow-x-auto">
                <div className="flex items-center gap-1.5">
                  {questions.map((q, idx) => {
                    ...
                    const isFull = validationResult?.questionEvaluations?.[q.id]?.isFullMatch;
              ...
              <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 sm:p-8 shadow-sm space-y-6 animate-in fade-in duration-150">
                ...
                <div className="space-y-3">
                  {currentActiveQuestion.options.map((opt) => {
                    const isSelected = (selectedAnswers[currentActiveQuestion.id] || []).includes(opt.id);
                    const isChecked = validationResult?.checked;
                    ...
                    return (
                      <div
                        key={opt.id}
                        onClick={() => handleToggleSelectLearner(currentActiveQuestion.id, opt.id)}
                        className={`p-4 rounded-xl border-2 transition-all flex items-center gap-3.5 ${isChecked
                      ...
                      >
                        <div className={`w-5 h-5 rounded-md flex items-center justify-center transition-colors ${isSelected ? 'bg-blue-600 text-white' : 'border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900'
                          }`}>
                        ...
                        {isChecked && isCorrect && isSelected && <CheckCircle2 size={18} className="text-emerald-600 shrink-0" />}
                        {isChecked && !isCorrect && isSelected && <AlertCircle size={18} className="text-rose-600 shrink-0" />}
                      </div>
                    );
                  })}
                </div>
            ...
            <div className="space-y-6 max-w-2xl mx-auto">
              {questions.map((q, qIndex) => (
                ...
                >
                  ...
                  <div className="space-y-2.5">
                    {q.options.map((opt) => {
                      ...
                      return (
                        <div
                          key={opt.id}
                          onClick={() => handleToggleSelectLearner(q.id, opt.id)}
                          className={`p-3.5 rounded-xl border-2 transition-all flex items-center gap-3.5 ${isChecked
                              ? isCorrect && isSelected
                                ? 'border-emerald-500 bg-emerald-50/60 dark:bg-emerald-950/40'
                                : !isCorrect && isSelected
                                  ? 'border-rose-500 bg-rose-50/60 dark:bg-rose-950/40'
                  ...
                  {validationResult?.checked && q.explanation && (
            ...
            <div>
              {validationResult ? (
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-500 mt-0.5">
                      {validationResult.correctPicks} of {totalCorrectCount} correct selections
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400">
                  Select your choices for all questions and click Check Answers.
                </p>
              )}
            </div>
            ...
            <div className="flex items-center gap-3">
              ...
              {(!validationResult || !validationResult.isFailed) && (
                <button
                  ...
                  disabled={Object.values(selectedAnswers).flat().length === 0}
          ...
          >
            ...
            <div className="space-y-2">
              ...
              <p className="text-[11px] text-slate-400 leading-normal">
                Format can be <code className="text-blue-600 dark:text-blue-400 font-semibold">A) Text</code>, <code className="text-blue-600 dark:text-blue-400 font-semibold">A. Text</code>, <code className="text-blue-600 dark:text-blue-400 font-semibold">1. Text</code>, or lines of text. Mark correct valid options with an asterisk (e.g. <code className="text-emerald-600 dark:text-emerald-400 font-semibold">*A) Option</code>).
              </p>
            </div>
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 max-w-2xl w-full shadow-2xl space-y-4">
            <div className="flex items-center justify-between pb-3 border-b border-slate-100 dark:border-slate-800">
              <div className="flex items-center gap-2.5 text-blue-600 dark:text-blue-400">
                ...
                <div>
                  <h3 className="text-sm font-bold text-slate-900 dark:text-white">
                    Bulk Paste Multiple Response Questions
                  </h3>
                  <p className="text-[11px] text-slate-500">
            ...
            <div className="space-y-2">
              ...
              <textarea
                ...
                placeholder={`1. Which of the following were major factors contributing to the decline of the Western Roman Empire?\n*A. Severe economic inflation and heavy taxation\n*B. Invasions and migrations by Germanic tribes\nC. Stable and unbroken peace across all provinces\n*D. Political corruption and military instability`}
              ...
              <div className="p-3 bg-slate-50 dark:bg-slate-950/70 border border-slate-200 dark:border-slate-800 rounded-xl space-y-1 text-[11px] text-slate-500 dark:text-slate-400">
                ...
                <p>• Automatically parses question numbers and choices (A, B, C, D...).</p>
                <p>• Prefix correct options with an asterisk (<code className="text-emerald-600 dark:text-emerald-400 font-mono">*A. Option</code>) to mark them as correct.</p>
              </div>
            </div>
