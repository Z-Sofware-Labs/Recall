...
; 7. File Associations Page
Var AssociateRecallCheckbox
Var AssociateRecallCheckboxState
...
Function PageFileAssociations
  ...
  !insertmacro MUI_HEADER_TEXT "File Associations" "Select file extensions to associate with Recall"
  ...
  ${NSD_CreateLabel} 0 0 100% 18u "Select the file types you want Recall to open by default:"
  Pop $0

  ${NSD_CreateCheckbox} 10u 24u 100% 14u "Associate .recall files (Recall Course Project Files)"
  Pop $AssociateRecallCheckbox

  ; Default to checked (1) on first load
  ${If} $AssociateRecallCheckboxState == ""
    StrCpy $AssociateRecallCheckboxState 1
  ${EndIf}

  ${If} $AssociateRecallCheckboxState == 1
    ${NSD_Check} $AssociateRecallCheckbox
  ${Else}
    ${NSD_Uncheck} $AssociateRecallCheckbox
  ${EndIf}

  ${NSD_CreateLabel} 22u 42u 88% 36u "When associated, double-clicking any .recall course project file in File Explorer will automatically launch and open it in Recall."
  Pop $0
...
Function PageLeaveFileAssociations
  ${NSD_GetState} $AssociateRecallCheckbox $AssociateRecallCheckboxState
FunctionEnd
...
Section Install
  ...
  ; Create file associations based on user choice
  ${If} $AssociateRecallCheckboxState == 1
  ${OrIf} ${Silent}
  ${OrIf} $PassiveMode = 1
  ...
  {{#each deep_link_protocols}}
    !insertmacro REGISTER_URL_SCHEME "{{this}}" "${PRODUCTNAME}" "$INSTDIR\${MAINBINARYNAME}.exe"
  {{/each}}
...
Section Uninstall
  ...
  ; Delete deep links
  {{#each deep_link_protocols}}
    !insertmacro UNREGISTER_URL_SCHEME "{{this}}"
  {{/each}}
